# app.py
# Red Diamond Project - Ultimate Production Version 3.0 (2026)
...
ALLOWED_DOMAINS = [
    ...
    "protonmail.com", "icloud.com", "zoho.com", "aol.com",
...
translations = {
    "ar": {
        ...
        "disposable": "بريد مؤقت غير مقبول ❌",
        "low_score": "موثوقية البريد ضعيفة ⚠️",
        "valid": "الإيميل صالح ✅",
        "fail": "فشل التحقق",
        "link_invalid": "رابط غير صالح ❌",
        "link_expired": "انتهت الصلاحية ⏰"
    },
    "en": {
        "no_email": "No email provided",
        "unsupported": "Unsupported email ❌",
        "disposable": "Disposable email ❌",
        "low_score": "Low reliability ⚠️",
...
def detect_location():
    ...
    try:
        ...
        return jsonify({"ip": ip, "country": "DZ", "success": False})

@app.route("/api/generate-unic", methods=["POST"])
...
def validate_user_email():
    ...
    try:
        r = requests.get(
            f"https://easyemailapi.com/api/verify/{email}",
...
def get_country():
    ...
    return jsonify({"country": country, "success": True})
